...
Any probability distributuion can be represented by a \emph{factor graph} and we will denote the probability of a given set of variables outputs $x$ on a graph $G$ as $P_G(x)$.
...
\begin{tikzpicture}[node distance = 2cm, auto]
  ...
  \path [line] (fSize) -- (size);
\end{tikzpicture}
\caption{Factor graph to model unconditional probability. It assumes independency between\dots}
\end{figure}
...
As possible to see in \autoref{fig:dora-data-simple} our approaximation to $P(x)$ althouth very basic was able to improve a novelty detection based only on conditional probability.


In order to generate a simple model we decided to model the unconditional density probability function also with a graphical model $G'$ and so we approximate $P(x)$ as $P_{G'}(x)$.
...
\item Use $P_G(x)$ as novelty threshold.
\end{enumerate}
