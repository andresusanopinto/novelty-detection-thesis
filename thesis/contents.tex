...
likelihood of a given instance of the $X$ variables in order to make a
\emph{MAP} operation on the graph to yield the desired value.
Such a factor would require tunning $2^N$ parameters, describing how to
distribute the weights over all the possible $2^N$ subsets of the $X$ variables.

Aiming at feasibility a simpler approach should be used.
Requiring 

This is the factor $\phi_N(X) = \prod_{i \in X} t_i$.
As so the factor $\phi_N$ can be decomposed into several $\phi_{N_i} = t_i$, one
for each variable, that measure on the threshold of detecting novelty on
the single variable $i$.

More formally, there is an interest in the new graph
Nonetheless after that a mechanism to allow to control the thresholds on
detecting novelty also needs to simple doing that the ability to control the thresholds on
variables is lost
In order to measure novelty of a set of variable the full set of approach
Aiming at obtaining a correct behaviour for detecting novelty on a certain
subset of variables
Optimally the system should re-evaluate novelty for all subsets of variables
each time new information is added.
A method to define a threshold on any subset of the $N$ variables also needs to
be introduced.

After that the system would be still be able to reason over variables on the
graph and deduce extra new novel variables if it was the case.
But such an approach would be dependent on the order of variables that the
system considers for novelty detection. And it would also suffer from inability
to reconsider the novelty on any given variable as more data becomes available.


The used approach can be extended to multiple variables. Though in order to
expand it to all the $n$ variables in the graph, $n!$ unconditional
probabilities would need to be evaluated.





Aiming at a fully probabilistic integration of novelty detection on any
number of variables the following method was introduced:
