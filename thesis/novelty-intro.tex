...
therefore it should proceed with caution.
...
examples of known cases. The lack of negative examples renders normal
...
the training data with statistic distributions and then perform tests whether a
given sample comes from the same distribution.

Parametric approaches: probabilistic /gmm approaches, hidden markov models,
hypothesis testing

Non-Parametric: knn Approachs, parzen density estimation, string matching,
clustering.
...
network has not seen before and the output of their system is then not expected
to be reliable.
...
\gls{K-PCA}~\citep{scholkopf1997kernel}, which uses the kernel-trick to extend
...
structures, the work of PLISS\dots 
