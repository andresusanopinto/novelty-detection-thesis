...
types increases exponential ammounts of data are needed to describe it.
...
Note than in the case there is only one sensed feature, the distribution
probability modelled by an assumption of independent features correctly models
unconditional probability. And it deviates from it as more features are sensed.
Though the individual factors associated with each variable try to unwrapped any
possible existent bias on each dimension and therefore it is expected to be a
