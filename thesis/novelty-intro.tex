...
Those samples contain different numbers of sensed properties for each room, mimicking
...
\caption{\label{fig:synthetic-roc}ROC curve comparing the novelty detection performance
         for samples with variable size of sensed properties.}
\end{figure}

The convex shape of the optimal threshold shows that the ratio between conditional
and unconditional probability is indeed an optimal detector \textbf{[Why does it show it's optimal?? Maybe too strong word?]}
and is suitable for implementing a threshold when the samples are taken from dynamic
