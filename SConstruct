#!/usr/bin/python

import os

VariantDir('build/semisupervised', 'semisupervised')
VariantDir('build/scripts', 'scripts')
VariantDir('build/pdis', 'pdis')
VariantDir('build/semantic-nd', 'semantic-nd')
VariantDir('build/thesis', 'thesis')
VariantDir('build/novelty-gm', 'novelty-gm')

SConscript('build/semisupervised/SConscript')
SConscript('build/semantic-nd/SConscript')
SConscript('build/pdis/presentation1/SConscript')
SConscript('build/pdis/presentation2/SConscript')
SConscript('build/pdis/techreport/SConscript')
SConscript('build/thesis/SConscript')
SConscript('build/novelty-gm/SConscript')


